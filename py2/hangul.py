...
def ChrHangul(hangul_index):
  ...
  return unichr(ord(FIRST_HANGUL_CHARACTER) + hangul_index)
...
def DecomposeHangul(hangul_character, jamo_romanizations):
  ...
  end_character = unichr(ord(FIRST_HANGUL_CHARACTER) + num_characters)
...
def AppendHangulPostfix(string, no_cons, cons, coding='utf-8'):
  """Append proper hangul postfix.

  Args:
    string: Hangul string, unicode or non-unicode.
    no_cons: Postfix for no final consonants such as 는, 가, 로, 를, 다
    cons: Post for final consonants such as 은, 이, 으로, 을, 이다
  """
  if type(string) is unicode:
    has_final = HasFinalConsonants(string)
  else:
    has_final = HasFinalConsonants(string.decode(coding))
  if has_final:
    return string + cons
  else:
...
def GetInitialCharacterBytes(string,
                             jamo_romanizations=JAMO_ROMANIZATIONS,
                             initial_jamos=INITIAL_JAMOS,
                             coding='utf-8'):
  """Wrapper function for GetInitialCharacter for (non-unicode) byte str.

  This is for python 2.x only.
  """
  if type(string) is str:
    unicode_string = string.decode(coding)
  else:
    unicode_string = string
  init_char = GetInitialCharacter(unicode_string,
                                  jamo_romanizations=jamo_romanizations,
                                  initial_jamos=initial_jamos)
  if type(init_char) is unicode:
    return init_char.encode(coding)
  else:
    return init_char
